...
const DeliveryActivities = () => {
    ...
    const getDeliveryActivities = async (from, dropValue) => {
        try {
            const response = await fetch(api.getDeliveryActivities(from, dropValue));
                        ...
                        {deliveryData.map((item, index) => (
                            <View key={index} style={styles(colors).cardView}>
                                <Text style={styles(colors).headingText}>
                                    {new Date(`${item.EntryDate}T${item.EntryTime}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </Text>
                                <View style={styles(colors).rowContainer}>
                                    <View style={styles(colors).rows}>
                                        <Text style={styles(colors).labelText}>Not Taken:</Text>
                                        <Text style={styles(colors).labelInnerText}>{item.NotTaken}</Text>
                                    </View>

                                    <View style={styles(colors).rows}>
                                        <Text style={styles(colors).labelText}>Not Verified:</Text>
                                        <Text style={styles(colors).labelInnerText}>{item.NotVerified}</Text>
                                    </View>
                                </View>

                                <View style={styles(colors).rowContainer}>
                                    <View style={styles(colors).rows}>
                                        <Text style={styles(colors).labelText}>Not Delivery:</Text>
                                        <Text style={styles(colors).labelInnerText}>{item.NotDelivery}</Text>
                                    </View>
                                    <View style={styles(colors).rows}>
                                        <Text style={styles(colors).labelText}>Overall Sales:</Text>
                                        <Text style={styles(colors).labelInnerText}>{item.OverAllSales}</Text>
                                    </View>
                                </View>
                            </View>
                        ))}
